...
# -*- coding: utf-8 -*-
import os
from PyQt5.QtWidgets import QWizard, QMessageBox, QWidget, QScrollArea, QLineEdit, QLabel, QVBoxLayout, QCheckBox, QGridLayout, QTableView, QHeaderView, QAbstractItemView
...
from ui.PersonProjectWizardUI import Ui_personProjectWizard
...
class PersonProjectWizard(QWizard):
    def __init__(self, parent=None):
        ...
        pluginList = self.loadConfiguredPlugins()
        if (pluginList):
            self.ProjectWizardPluginListModel = PluginConfigurationListModel(pluginList, self)
            self.ProjectWizardPluginListModel.checkPluginConfiguration()

        # Connect signals
        self.ui.personProjectSearchButton.clicked.connect(self.searchForTargets)
        self.ui.personProjectSearchForValue.returnPressed.connect(self.searchForTargets)
