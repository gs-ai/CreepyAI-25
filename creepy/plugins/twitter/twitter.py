...
from PyQt5.QtWebEngineWidgets import QWebEngineView  # Updated import
...
class Twitter(InputPlugin):
            ...
            if search_results:
                ...
                for i in search_results:
                    ...
                    if not os.path.exists(temp_file):
                        urllib.request.urlretrieve(i.profile_image_url, temp_file)
    ...
    def getAuthenticatedAPI(self):
        try:
            auth = tweepy.OAuthHandler(self.options_string['hidden_application_key'], self.options_string['hidden_application_secret'])
    ...
    def runConfigWizard(self):
        try:
            ...
            authorizationURL = oAuthHandler.get_authorization_url()
            self.wizard = QWizard()
            page1 = QWizardPage()
            page2 = QWizardPage()
            layout1 = QVBoxLayout()
            ...
            label1a = QLabel("Click next to connect to twitter.com. Please login with your account and follow the instructions to authorize Creepy.")
            label2a = QLabel("Copy the PIN you receive after authorizing Creepy and enter it below, then click finish.")
            pinLabel = QLabel("PIN")
            inputPin = QLineEdit()
            inputPin.setObjectName("inputPin")
            
            html = QWebEngineView()  # Updated to QWebEngineView
            ...
            if self.wizard.exec_():
                try:
                    oAuthHandler.get_access_token(inputPin.text().strip())
                    access_token = oAuthHandler.access_token
                    access_token_secret = oAuthHandler.access_token_secret
                    self.options_string['hidden_access_token'] = access_token
                    self.options_string['hidden_access_token_secret'] = access_token_secret
                    self.config.write()
                except Exception as err:
                    logger.error("Error completing the wizard: %s", err)
                    self.showWarning("Error completing the wizard", "Unable to obtain the access token for your account. Please try to run the wizard again.")
        except Exception as err:
            logger.exception("Exception in runConfigWizard: %s", err)
    ...
    def isConfigured(self):
        ...
        try:
            ...
            logger.debug("Authenticated user: %s", api.me().name)
            return (True, "")
        except Exception as e:
            logger.error("Error authenticating with Twitter API: %s", e)
            return (False, "Error authenticating with Twitter: " + str(e))
    ...
    def constructContextInfoWindow(self, tweet):
        ...
        html = str(self.options_string['infowindow_html'])
