...
from utilities.error_handling import setup_exception_handling, catch_exceptions, ui_exception_handler, ErrorTracker
...
class MainWindow(QMainWindow):
    ...
    class analyzeProjectThread(QThread):
        ...
        def run(self):
            try:
                ...
                # Track total plugins and processed plugins for progress updates
                total_targets = len(self.project.selectedTargets)
                processed_targets = 0
                
                for target in self.project.selectedTargets:
                    try:
                        plugin = pluginManager.getPluginByName(target['pluginName'], 'Input')
                        if not plugin:
                            logger.error(f"Plugin {target['pluginName']} not found")
                            continue
                            
                        pluginObject = plugin.plugin_object
                        runtimeConfig = next((pl['searchOptions'] for pl in self.project.enabledPlugins 
                                            if pl['pluginName'] == target['pluginName']), None)
                        
                        # Check if plugin is configured
                        is_configured, _ = pluginObject.isConfigured()
                        if not is_configured:
                            logger.warning(f"Plugin {target['pluginName']} is not configured properly")
                            
                        # Get locations from the plugin
                        ...
                        logger.error(f"Error processing target {target.get('targetUsername', 'unknown')}: {str(e)}")
                        error_tracker.track_error(type(e), str(e), f"Plugin: {target.get('pluginName', 'unknown')}")
                ...
                error_tracker.track_error(type(e), str(e), "analyzeProjectThread")
                self.project.isAnalysisRunning = False
                self.locations_signal.emit(self.project)
    ...
    def __init__(self, parent=None):
        ...
        # Initialize plugin error tracking system
        self.plugin_errors = {}

    @ui_exception_handler
    def _setup_connections(self):
        """Setup all the UI signal connections"""
    ...
    def loadConfiguredPlugins(self):
        ...
        try:
            ...
            # Activate all plugins and log their status
            for plugin in self.PluginManager.getAllPlugins():
                try:
                    plugin.plugin_object.activate()
                    is_configured, message = plugin.plugin_object.isConfigured()
                    logger.info(f"Plugin '{plugin.name}' loaded. Configured: {is_configured}, Status: {message}")
                except Exception as e:
                    logger.error(f"Error activating plugin '{plugin.name}': {str(e)}")
                    error_tracker.track_error(type(e), str(e), f"Plugin: {plugin.name}")
                    
        except Exception as e:
            self.showWarning('Error', f'Failed to load plugins: {str(e)}')
            error_tracker.track_error(type(e), str(e), "loadConfiguredPlugins")
